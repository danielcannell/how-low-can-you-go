...
void fragment() {
    ...
    float intensity = pow(sin(10.0 * uv.y + sin(20.0 * uv.x + 0.7 * TIME) + 0.3 * TIME), 4.0);
    intensity = 0.4 + 0.6 * intensity;
    intensity *= texture(TEXTURE, uv).a;

    vec3 base_col = texture(NORMAL_TEXTURE, uv).xyz;

    float blue = 0.6 + 0.4 * pow(sin(uv.x + 0.1 * TIME), 2.0);
    float green = 0.6 + 0.4 * pow(sin(uv.y + 1.1 * 0.1 * TIME), 2.0);
